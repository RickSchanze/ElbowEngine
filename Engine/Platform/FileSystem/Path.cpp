...
static bool IsProjectPathValid(core::StringView path) {
  // path必须是一个文件夹
  if (!platform::Path::IsFolder(path))
    return false;
...
bool platform::Path::SetProjectPath(core::StringView path) {
  ...
  if (!IsProjectPathValid(path)) {
    LOGGER.Error(logcat::Platform_FileSystem,
                 "The project path must either be an empty folder or contain an .elbowengine file.");
    return false;
  }
  s_proj_path_ = path;
...
bool platform::Path::IsExist(core::StringView path) {
  return exists(std::filesystem::path(core::String(path).ToWideString()));
}
