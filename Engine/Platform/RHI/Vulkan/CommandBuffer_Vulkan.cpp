...
CommandPool_Vulkan::CommandPool_Vulkan(CommandPoolCreateInfo info) : CommandPool(info) {
  ...
  case QueueFamilyType::Compute:
    throw core::NotSupportException("计算队列尚不支持");
    break;
...
core::Array<core::SharedPtr<CommandBuffer>> CommandPool_Vulkan::CreateCommandBuffers(uint32_t count,
                                                                                     bool self_managed) {
...
void CommandBuffer_Vulkan::Reset() { vkResetCommandBuffer(buffer_, 0); }
...
static void ExecuteCmdCopyBuffer(VkCommandBuffer cmd, Cmd_CopyBuffer *cmd_copy_buffer) {
  ...
  if (cmd_copy_buffer->src && cmd_copy_buffer->dst) {
    ...
    vkCmdCopyBuffer(cmd, cmd_copy_buffer->src->GetNativeHandleT<VkBuffer>(),
                    cmd_copy_buffer->dst->GetNativeHandleT<VkBuffer>(), 1, &copy_region);
...
static void ExecuteCmdBindPipeline(VkCommandBuffer cmd, Cmd_BindPipeline *cmd_bind_pipeline) {
  PROFILE_SCOPE_AUTO;
  if (cmd_bind_pipeline == nullptr || cmd_bind_pipeline->pipeline == nullptr ||
      !cmd_bind_pipeline->pipeline->IsValid()) {
...
static void ExecuteCmdBindVertexBuffer(VkCommandBuffer cmd, Cmd_BindVertexBuffer *cmd_bind_vertex_buffer) {
  PROFILE_SCOPE_AUTO;
  if (cmd_bind_vertex_buffer == nullptr || cmd_bind_vertex_buffer->buffer == nullptr ||
      !cmd_bind_vertex_buffer->buffer->IsValid()) {
...
static void ExecuteCmdBindIndexBuffer(VkCommandBuffer cmd, Cmd_BindIndexBuffer *cmd_bind_index_buffer) {
  PROFILE_SCOPE_AUTO;
  if (cmd_bind_index_buffer == nullptr || cmd_bind_index_buffer->buffer == nullptr ||
      !cmd_bind_index_buffer->buffer->IsValid()) {
...
static void ExecuteCmdImagePipelineBarrier(VkCommandBuffer cmd, Cmd_ImagePipelineBarrier *cmd_image_pipeline_barrier) {
  PROFILE_SCOPE_AUTO;
  if (cmd_image_pipeline_barrier == nullptr || cmd_image_pipeline_barrier->target == nullptr ||
      !cmd_image_pipeline_barrier->target->IsValid()) {
  ...
  barrier.subresourceRange.aspectMask =
      RHIImageAspectToVkImageAspect(cmd_image_pipeline_barrier->subresource_range.aspect_mask);
...
static void ExecuteCmdBindDescriptorSet(VkCommandBuffer cmd, Cmd_BindDescriptorSet *cmd_bind_descriptor_set) {
  if (cmd_bind_descriptor_set == nullptr || cmd_bind_descriptor_set->set == nullptr ||
      !cmd_bind_descriptor_set->set->IsValid()) {
...
static void ExecuteCmdCopyBufferToImage(VkCommandBuffer cmd, Cmd_CopyBufferToImage *cmd_copy_buffer_to_image) {
  if (cmd_copy_buffer_to_image == nullptr || cmd_copy_buffer_to_image->src == nullptr ||
      !cmd_copy_buffer_to_image->src->IsValid() || cmd_copy_buffer_to_image->dst == nullptr ||
      !cmd_copy_buffer_to_image->dst->IsValid()) {
  ...
  copy_region.imageSubresource.aspectMask =
      RHIImageAspectToVkImageAspect(cmd_copy_buffer_to_image->subresource_range.aspect_mask);
  ...
  copy_region.imageOffset = {cmd_copy_buffer_to_image->offset.x, cmd_copy_buffer_to_image->offset.y,
                             cmd_copy_buffer_to_image->offset.z};
  VkExtent3D extent{};
