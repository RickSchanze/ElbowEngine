...
static SwapChainSupportInfo QuerySwapChainSupportInfo(const VkPhysicalDevice device, const Surface *surface) {
  ...
  if (format_count != 0) {
    ...
    info.formats = formats | Transform([](const VkSurfaceFormatKHR &format) {
                     return SurfaceFormat{.format = VkFormatToRHIFormat(format.format),
                                          .color_space = VkColorSpaceToRHIColorSpace(format.colorSpace)};
                   }) |
                   ToArray;
  }
  ...
  if (present_mode_count != 0) {
    ...
    info.present_modes =
        present_modes | Transform([](VkPresentModeKHR mode) { return VkPresentModeToRHIPresentMode(mode); }) | ToArray;
  }
  return info;
}
...
#if ELBOW_DEBUG
  if (core::GetConfig<PlatformConfig>()->GetValidEnableValidationLayer() == false)
    return;
...
VkQueue GfxContext_Vulkan::GetQueue(QueueFamilyType type) const {
  ...
  case QueueFamilyType::Compute:
    throw core::NotSupportException("Compute queue尚不支持");
    break;
...
Format GfxContext_Vulkan::FindSupportedFormat(const core::Array<Format> &candidates, VkImageTiling tiling,
                                              VkFormatFeatureFlags features) const {
  for (const core::Array<VkFormat> fmts =
           candidates | Transform([](Format fmt) { return RHIFormatToVkFormat(fmt); }) | ToArray;
       const auto fmt : fmts) {
...
void GfxContext_Vulkan::FindVulkanExtensionSymbols() {
  ...
  CmdBeginDebugUtilsLabelEXT =
      reinterpret_cast<PFN_vkCmdBeginDebugUtilsLabelEXT>(vkGetDeviceProcAddr(device_, "vkCmdBeginDebugUtilsLabelEXT"));
  CmdEndDebugUtilsLabelEXT =
      reinterpret_cast<PFN_vkCmdEndDebugUtilsLabelEXT>(vkGetDeviceProcAddr(device_, "vkCmdEndDebugUtilsLabelEXT"));
}
...
core::UniquePtr<GraphicsPipeline> GfxContext_Vulkan::CreateGraphicsPipeline(const GraphicsPipelineDesc &create_info,
...
GfxContext_Vulkan *GetVulkanGfxContext() { return static_cast<GfxContext_Vulkan *>(GetGfxContext()); }
...
static void CreateInstance(core::Ref<VkInstance> instance) {
  ...
  app_info.applicationVersion = VK_MAKE_VERSION(core_cfg->GetAppVersion().major, core_cfg->GetAppVersion().minor,
...
static bool CheckDeviceExtensionSupport(VkPhysicalDevice device) {
  ...
  auto filter_exts =
      my_required_extensions | //
      RemoveIf([&extensions](const core::String &ext) {
        return AnyOf(extensions, [&ext](const VkExtensionProperties &prop) { return ext == prop.extensionName; });
      });
...
static QueueFamilyIndices FindQueueFamilies(VkPhysicalDevice device, const Surface *surface) {
  ...
  for (const auto &queue_family : queue_families) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(device, i, static_cast<VkSurfaceKHR>(surface->GetNativeHandle()),
                                         &present_support);
    if (present_support) {
...
static void SelectPhysicalDevice(core::Ref<VkPhysicalDevice> physical_device, VkInstance instance, Surface *surface) {
  ...
  core::Assert::Require(logcat::Platform_RHI_Vulkan, physical_device != VK_NULL_HANDLE,
                        "Failed to find a suitable GPU!");
}

static void CreateLogicalDevice(VkPhysicalDevice physical_device, Surface *surface, OUT VkDevice &device,
                                OUT VkQueue &graphics_queue, OUT VkQueue &present_queue, OUT VkQueue &transfer_queue) {
...
static Format CreateSwapChain(const SwapChainSupportInfo &swapchain_support, const Surface *surface,
                              VkPhysicalDevice physical_device, VkDevice device, ImageDesc &desc, core::Size2D size,
  ...
  core::Assert::Require(logcat::Platform_RHI_Vulkan,
                        image_count <= swapchain_support.max_image_count &&
                            image_count >= swapchain_support.min_image_count,
...
GfxContext_Vulkan::GfxContext_Vulkan() {
  ...
  default_color_format_ = CreateSwapChain(QuerySwapChainSupportInfo(), surface_, physical_device_, device_,
                                          swapchain_image_desc_, cfg->GetDefaultWindowSize(), swapchain_);
  Format depth_format =
      FindSupportedFormat({Format::D32_Float, Format::D32_Float_S8X24_UInt, Format::D24_UNorm_S8_UInt},
                          VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
  core::Assert::Require(logcat::Platform_RHI_Vulkan, depth_format != Format::Count,
...
void GfxContext_Vulkan::ResizeSwapChain(Int32 width, Int32 height) {
  ...
  default_color_format_ = CreateSwapChain(QuerySwapChainSupportInfo(), surface_, physical_device_, device_,
                                          swapchain_image_desc_, Size2D{width, height}, swapchain_);
  uint32_t img_cnt = 0;
