...
GraphicsPipeline_Vulkan::GraphicsPipeline_Vulkan(const GraphicsPipelineDesc &desc, const RenderPass *render_pass) {
  ...
  Array<VkDescriptorSetLayout> layouts_native = desc.descriptor_set_layouts |
                                                views::transform([](const SharedPtr<DescriptorSetLayout> &layout) {
  ...
  if (render_pass == nullptr) {
    ...
    Array<VkFormat> color_formats = desc.attachments.color_formats |
                                    views::transform([](Format format) { return RHIFormatToVkFormat(format); }) |
                                    to<Array>();
