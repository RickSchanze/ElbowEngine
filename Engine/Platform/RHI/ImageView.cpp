...
platform::rhi::ImageViewDesc::ImageViewDesc(Image *image_, const ImageDimension type_, const Format format_,
                                            const ImageSubresourceRange &subresource_range_,
                                            const ComponentMapping component_mapping_)
    : image(image_), type(type_), format(format_), subresource_range(subresource_range_),
      component_mapping(component_mapping_) {
  if (image == nullptr) {
  ...
  }
  {
    ...
    } else if (type == ImageDimension::Array1D || type == ImageDimension::Array2D ||
               type == ImageDimension::ArrayCube) {
