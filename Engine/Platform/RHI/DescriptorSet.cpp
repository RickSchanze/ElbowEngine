...
size_t DescriptorSetLayoutDesc::GetHashCode() const {
  ...
  for (const auto &binding : bindings) {
    ...
    binding_hash ^=
        hasher_uint32(GetEnumValue(binding.descriptor_type)) + 0x9e3779b9 + (binding_hash << 6) + (binding_hash >> 2);
...
void DescriptorSetLayoutPool::Clear(GfxContext *) { pool_.clear(); }
...
Array<SharedPtr<DescriptorSet>> DescriptorSetPool::Allocates(const Array<SharedPtr<DescriptorSetLayout>> &layouts) {
  Array<DescriptorSetLayout *> layout_ptrs =
      layouts | ranges::views::transform([](const auto &layout) { return layout.get(); }) | ranges::to<Array>;
