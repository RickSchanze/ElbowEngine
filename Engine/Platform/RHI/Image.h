...
struct ImageDesc {
  ...
  ImageDesc(const size_t width_, const size_t height_, const ImageUsage usage_, const Format format_,
            const ImageDimension dimension_ = ImageDimension::D2, const uint16_t depth_or_layers_ = 1,
            const uint16_t mip_levels_ = 1, const SampleCount samples_ = SampleCount::SC_1,
            const ImageLayout initial_state_ = ImageLayout::Undefined)
      : width(width_), height(height_), usage(usage_), dimension(dimension_), depth_or_layers(depth_or_layers_),
        mip_levels(mip_levels_), format(format_), samples(samples_), initial_state(initial_state_) {}
...
public:
   ...
   * @return
   */
  [[nodiscard]] virtual core::SharedPtr<Buffer> CreateCPUVisibleBuffer() = 0;
...
template <> struct std::hash<platform::rhi::SamplerDesc> {
