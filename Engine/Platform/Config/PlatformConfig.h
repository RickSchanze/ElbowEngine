...
namespace platform {
class CLASS(Config = "Config/Platform/PlatformConfig.cfg", Category = "Platform") PlatformConfig
    : extends core::IConfig {
  ...
  PROPERTY(Label = "是否开启DynamicRendering特性", EnableWhen = "api == GraphicsAPI::Vulkan",
           Category = "Platform|RHI|Vulkan")
  DECLARE_CONFIG_ITEM(bool, enable_dynamic_rendering, EnableDynamicRendering, true);

public:
  [[nodiscard]] uint8_t GetValidFrameCountInFlight() const;

  [[nodiscard]] bool GetValidEnableValidationLayer() const;
};
} // namespace platform
