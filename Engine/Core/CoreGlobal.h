...
template <typename T, typename... Args> T *NewPooled(core::MemoryPool *pool, Args &&...args) {
...
template <typename T, typename... Args> T *NewPooledId(const Int32 id, Args &&...args) {
...
template <typename T> void DeletePooled(core::MemoryPool *pool, T *p) {
  p->~T();
  _MemoryPoolFree(pool, p);
}

template <typename T> void DeletePooledId(const Int32 id, T *p) {
  auto pool = _GetMemoryPool(id);
  DeletePooled(pool, p);
}

template <typename T> void Delete(T *p) { DeletePooledId(0, p); }

inline void *Malloc(UInt64 size) { return mi_malloc(size); }
inline void Free(void *ptr) { mi_free(ptr); }

// 编译时获得常量字符串长度
template <std::size_t N> constexpr std::size_t STRLEN(const char (&str)[N]) {
