...
template <int Index, typename Receiver, typename Sender> struct WhenAllSubReceiver {
  ...
  template <typename E> friend auto TagInvoke(SetErrorType, WhenAllSubReceiver &&r, E &&e) noexcept {
...
template <typename Receiver, typename... Senders> struct WhenAllReceiver {
  ...
  template <int Index, typename Tuple> void SetValue(Tuple &&tuple) {
    try {
  ...
  template <typename E> friend void TagInvoke(SetErrorType, WhenAllReceiver &&r, E &&e) noexcept {
    SetError(std::move(r.receiver), Forward<E>(e));
  }
...
template <typename Receiver, typename... Senders> struct WhenAllOperation {
  std::shared_ptr<Pure<Receiver>> receiver;
  ...
  template <int Index = 0> void ApplyStart() {
...
template <typename... Senders> struct WhenAllSender {
...
struct WhenAllType {
  template <typename... Senders> auto operator()(Senders &&...senders) const noexcept {
