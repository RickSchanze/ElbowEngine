  ...
  for (auto field : fields) {
    if (field->IsAssociativeContainer()) {
      ...
      if (enum_op_value) {
        ...
        LOGGER.Error(logcat::Archive_Serialization, "Serialize filed {} failed: get enum value failed!",
                     field->GetName());
      }
    } else {
      emitter << YAML::Key << field->GetName().Data() << YAML::Value;
...
bool YamlArchive::Serialize(const Any &obj, String &out) {
  ...
  if (obj.GetType()->IsDefined(Type::Interface)) {
    LOGGER.Error(logcat::Archive_Serialization, "Serialize interface [type = {}] is not allowed.",
...
static bool DeserializeNode(const YAML::Node &node, void *out, const Type *type) {
  if (type->IsEnum()) {
    ...
    LOGGER.Error(logcat::Archive_Deserialization, "Deserialize field failed, type = {}, failed to find enum value: {}",
                 type->GetFullName(), key_str);
    return false;
  }
  ...
  for (auto field : fields) {
    ...
    if (!field_node.IsDefined()) {
      LOGGER.Warn(logcat::Archive_Deserialization,
                  "Deserialize field {} failed, type = {}, node is not defined, use default value instead", field_name,
                  type->GetFullName());
      continue;
    }
    ...
    if (field->IsSequentialContainer()) {
      ...
      if (container == nullptr) {
        LOGGER.Error(logcat::Archive_Deserialization,
                     "Deserialize field {} failed, type = {}, failed to create valid sequence view", field_name,
                     type->GetFullName());
        return false;
      }
      ...
      if (container == nullptr) {
        LOGGER.Error(logcat::Archive_Deserialization,
                     "Deserialize field {} failed, type = {}, failed to create valid associative view", field_name,
                     type->GetFullName());
        return false;
      }
      ...
      if (!field->GetType()->IsEnum()) {
        LOGGER.Error(logcat::Archive_Deserialization,
                     "Deserialize field {} failed. When deserialize a enum, field->IsEnum() == true and "
                     "field_type->IsEnum() == true must be "
                     "satisfied.",
                     field_name);
        return false;
      };
      ...
      if (!enum_op) {
        LOGGER.Error(logcat::Archive_Deserialization, "Deserialize field {} failed, type = {}, enum value {} not found",
                     field_name, type->GetFullName(), enum_str);
        return false;
      }
...
bool YamlArchive::Deserialize(StringView source, void *out, const Type *type) {
  ...
  if (type->IsDefined(Type::Interface)) {
    LOGGER.Error(logcat::Archive_Deserialization, "Deserialize interface is not allowed, type: {}",
                 type->GetFullName());
    return false;
  }
