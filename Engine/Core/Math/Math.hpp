...
struct Math {
    static bool ApproximatelyEqual(const float a, const float b, const float tolerance = 1.e-4f) { return std::fabs(a - b) <= tolerance; }
    ...
    static Float Radians(const Float a) { return a * Pi() / 180.f; }
    ...
    static Matrix4x4f LookAt(Vector3f pos, Vector3f target, Vector3f up);
};
