...
namespace core {
template <typename ReturnT, typename... ArgumentArgs> struct Event {
  ...
  template <typename Class> void Bind(Class *obj, ReturnType (Class::*func)(ArgumentArgs...)) {
    // DebugAssert(Event, !HasBound(), "Event has been bound, rebinding.");
    delegate_ = Move(DelegateType([obj, func](ArgumentArgs &&...args) { return (obj->*func)(Move(args)...); }));
  }

  template <typename Class> void Bind(Class *obj, ReturnType (Class::*func)(ArgumentArgs...) const) {
    // DebugAssert(Event, !HasBound(), "Event has been bound, rebinding.");
    delegate_ = Move(DelegateType([obj, func](ArgumentArgs &&...args) { return (obj->*func)(Move(args)...); }));
  }
 ...
 */
template <typename ReturnT, typename... ArgumentArgs> struct MulticastEvent {
  using ReturnType = ReturnT;
  using DelegateType = Delegate<ReturnType, ArgumentArgs...>;
  ...
  DelegateID AddBind(DelegateType &&delegate) {
    // DebugAssert(Event, !ContainerUtils::Contains(delegates_, delegate), "Event has already bind delegate {}",
    // delegate.GetID());
  ...
  template <typename Class> DelegateID AddBind(Class *obj, ReturnType (Class::*func)(ArgumentArgs...)) {
    DelegateType delegate([obj, func](ArgumentArgs... args) { return (obj->*func)(args...); });
    return AddBind(Move(delegate));
  }

  template <typename Class> DelegateID AddBind(Class *obj, ReturnType (Class::*func)(ArgumentArgs...) const) {
    DelegateType delegate([obj, func](ArgumentArgs... args) { return (obj->*func)(args...); });
    return AddBind(Move(delegate));
  }
...
#define REGISTER_EVENT_HANDLER_BEFORE_MAIN()                                                                           \
  ...
  namespace {                                                                                                          \
  struct STATIC_INIT {                                                                                                 \
    STATIC_INIT() { REGISTER_EVENT_HANDLERS(); }                                                                       \
  };                                                                                                                   \
