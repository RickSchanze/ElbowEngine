...
public:
  // 构造函数
  template <typename... Args> static UniquePtr<T> Create(Args &&...args) {
  ...
  // 子类到父类的转换
  template <typename U, typename = EnableIfType<IsBaseOf<T, U>>> UniquePtr(UniquePtr<U> &&other) noexcept : ptr_(other.Release()) {}
...
template <typename T, typename... Args> UniquePtr<T> MakeUnique(Args &&...args) { return UniquePtr<T>::Create(Forward<Args>(args)...); }