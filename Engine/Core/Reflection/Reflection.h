...
struct FieldInfo {
  ...
  [[nodiscard]] bool IsSequentialContainer() const {
    return container_identifier_ == ContainerIdentifier::Array ||
           container_identifier_ == ContainerIdentifier::StaticArray ||
...
struct FunctionInfo {
  ...
  template <typename ReturnT, typename ObjectT> [[nodiscard]] Optional<ReturnT> Invoke(ObjectT *obj) {
  ...
  template <typename ObjT> bool AddParam(ObjT param) {
...
template <typename ReturnT, typename... Args> struct FunctionInfoImpl : FunctionInfo {
  Any InvokeImpl(void *) override { return InvokeHelper(::std::index_sequence_for<Args...>{}); }

  template <size_t... Is> Any InvokeHelper(::std::index_sequence<Is...>) {
    try {
      if (params.size() != param_infos.size())
        return {};
...
template <typename ReturnT, typename ClassT, typename... Args> struct MemberFunctionImpl : FunctionInfo {
  Any InvokeImpl(void *input) override {
    return InvokeHelper(static_cast<ClassT *>(input), ::std::index_sequence_for<Args...>{});
  }

  template <size_t... Is> Any InvokeHelper(ClassT *obj, ::std::index_sequence<Is...>) {
    try {
      if (params.size() != param_infos.size())
        return {};
...
#define REGISTER_FIELD_IMPL(name)                                                                                      \
    ...
    info.container_view_ =                                                                                             \
        New<DynamicArrayView<ClassT, typename ContainerTypeTrait<T>::ValueType, name>>(field, this);                   \
  }
  // clang-format off
    /**
...
/**
 * 解析一个属性到子属性
 * PROPERTY(SQLAttr="(PrimaryKey, AutoIncrement, SomeOther=(Test=P))") -> { "PrimaryKey": true, "AutoIncrement": true,
 * "SomeOther": {"Test": "P" } } 不加等号默认为true 目前只支持一层嵌套, 也就是上面的SomeOther=里面的内容不支持
 * TODO: 多层嵌套支持
 * @param attr
 * @return
 */
...
inline core::StringView GetEnumStringFieldValueAttribute(core::FieldInfo::ValueAttribute value) {
  switch (value) {
  case core::FieldInfo::ValueAttribute::Getter:
    return "Getter";
  case core::FieldInfo::ValueAttribute::Setter:
    return "Setter";
  case core::FieldInfo::ValueAttribute::Label:
    return "Label";
  default:
    return "OutOfRange";
  }
