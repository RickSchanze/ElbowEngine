    ...
    Ext_AddField(const StringView name, FiledT ClassT::* class_field, UInt64 offset, const Type* outer_t)
    {
        ...
        else
        {
            ...
            field->container_view =
                New<MapView<ClassT, typename ContainerTypeTrait<FiledT>::KeyType, typename ContainerTypeTrait<FiledT>::ValueType, Map> >(class_field,
                                                                                                                                         outer_t);
        }
    }
...
inline Ext_TypeFlagAttribute Attribute(const Type::FlagAttribute attr)
{
    return {attr};
}

inline Ext_TypeValueAttribute Attribute(const Type::ValueAttribute attr, const StringView value)
{
    return {attr, value};
}
...
inline bool IsNumericFloat(const Type* t)
{
...
{
    return t == TypeOf<String>() || t == TypeOf<StringView>();
}
