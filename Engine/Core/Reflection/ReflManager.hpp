...
static void EnumConstructor(void *ptr) { new (ptr) Int32(); }
static void EnumDestructor(void *ptr) { static_cast<Int32 *>(ptr)->~Int32(); }
...
public:
    ...
    [[nodiscard]] Float GetLevel() const override { return 2; }
    [[nodiscard]] StringView GetName() const override { return "ReflManager"; }

private:
    Map<RTTITypeInfo, Type *> types_registered_;
    Map<RTTITypeInfo, MetaDataRegisterer> meta_data_registers_;
    Map<RTTITypeInfo, CtorDtor> ctors_;
};
...
inline size_t std::hash<RTTITypeInfo>::operator()(const RTTITypeInfo &type) const noexcept { return type.hash_code; }
