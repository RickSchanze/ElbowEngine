...
FieldInfo::FieldInfo(FieldInfo &&info) noexcept
    : offset_(info.offset_), size_(info.size_), name_(info.name_), enum_value_(info.enum_value_),
      attribute_(info.attribute_), value_attr_(info.value_attr_), container_view_(Move(info.container_view_)),
      type_(info.type_), outer_(info.outer_), container_identifier_(info.container_identifier_) {}

bool FieldInfo::IsPrimitive() const { return type_ && type_->IsPrimitive(); }

bool FieldInfo::IsEnum() const { return type_ && type_->IsEnum(); }
...
Any FieldInfo::GetValue(const Any &obj) const {
  ...
  if (!obj.IsDerivedFrom(outer_)) {
    LOGGER.Error(logcat::Archive_Serialization, "Different outer type, obj type: {}, outer type: {}",
                 obj.GetType()->GetFullName(), outer_->GetFullName());
    return {};
  }
...
static void CastInteger(void *target, const Type *t, int64_t v) {
  if (t == TypeOf<int8_t>())
    *static_cast<int8_t *>(target) = static_cast<int8_t>(v);
  if (t == TypeOf<uint8_t>())
    *static_cast<uint8_t *>(target) = static_cast<uint8_t>(v);
  if (t == TypeOf<int16_t>())
    *static_cast<int16_t *>(target) = static_cast<int16_t>(v);
  if (t == TypeOf<uint16_t>())
    *static_cast<uint16_t *>(target) = static_cast<uint16_t>(v);
  if (t == TypeOf<int32_t>())
    *static_cast<int32_t *>(target) = static_cast<int32_t>(v);
  if (t == TypeOf<uint32_t>())
    *static_cast<uint32_t *>(target) = static_cast<uint32_t>(v);
  if (t == TypeOf<int64_t>())
    *static_cast<int64_t *>(target) = static_cast<int64_t>(v);
  if (t == TypeOf<uint64_t>())
    *static_cast<uint64_t *>(target) = static_cast<uint64_t>(v);
}
...
void FieldInfo::SetValue(const Any &obj, const Any &value) const {
  ...
  if (type_->IsEnum()) {
    *static_cast<Int32 *>(GetFieldPtr(obj.GetRawPtr())) = *static_cast<int32_t *>(value.GetRawPtr());
    return;
  }
...
const Type *GetObjectPtrBaseType() { return TypeOf<ObjectPtrBase>(); }
...
bool Type::IsAttributeValueNull(const ValueAttribute attr) const { return GetAttributeValue(attr) == "-"; }

bool Type::IsNumericInteger() const {
  return this == TypeOf<int8_t>() || this == TypeOf<uint8_t>() || this == TypeOf<int16_t>() ||
         this == TypeOf<uint16_t>() || this == TypeOf<int32_t>() || this == TypeOf<uint32_t>() ||
         this == TypeOf<int64_t>() || this == TypeOf<uint64_t>();
}

bool Type::IsNumericFloat() const { return this == TypeOf<float>() || this == TypeOf<double>(); }

bool Type::IsBoolean() const { return this == TypeOf<bool>(); }

bool Type::IsString() const { return this == TypeOf<String>(); }

StringView Type::GetName() const { return name_.SubString(name_.LastIndexOf(".") + 1, name_.Length()); }
...
bool Type::HasSelfDefinedMemberFunction(StringView name) const {
  return std::ranges::any_of(GetSelfDefinedMemberFunctions(),
                             [name](const FunctionInfo *func) { return func->GetName() == name; });
}
...
bool Type::HasMemberFunction(StringView name) const {
  return std::ranges::any_of(GetMemberFunctions(),
                             [name](const FunctionInfo *func) { return func->GetName() == name; });
}

bool Type::IsPrimitive() const {
  if (this == TypeOf<int8_t>())
    return true;
  if (this == TypeOf<uint8_t>())
    return true;
  if (this == TypeOf<int16_t>())
    return true;
  if (this == TypeOf<uint16_t>())
    return true;
  if (this == TypeOf<int32_t>())
    return true;
  if (this == TypeOf<uint32_t>())
    return true;
  if (this == TypeOf<int64_t>())
    return true;
  if (this == TypeOf<uint64_t>())
    return true;
