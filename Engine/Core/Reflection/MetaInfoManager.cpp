...
void MetaInfoManager::RegisterType(const RTTITypeInfo &type_info) {
  if (types_registered_.contains(type_info)) {
    return;
  }
  if (!meta_data_registers_.contains(type_info)) {

    for (auto &val : meta_data_registers_ | std::views::keys) {
      LOGGER.Info(logcat::Test, "Existing register: {}", val.name);
    }
