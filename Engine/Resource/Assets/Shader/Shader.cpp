  ...
  if (field_cnt == 0) {
    ...
    for (UInt32 i = 0; i < field_cnt; ++i) {
      ...
      if (param_type == ShaderParamType::SamplerState || param_type == ShaderParamType::Texture2D) {
        LOGGER.Error(logcat::Resource, "{}:{}:{} 结构体参数成员中不能有资源类型, 请将其写到顶层", path, variable_name,
                     GetEnumValue(param_type));
        return false;
      } else {
...
void Shader::GetParams(Array<ShaderParam> &out, bool &has_camera) {
  ...
  if (!success) {
    out.clear();
    LOGGER.Error(logcat::Resource,
                 "Shader::GetParams: 解析参数错误, 是否有非Texture和Sampler参数未被ConstantBuffer包围?");
  }
}
...
void Shader::Compile(bool output_glsl) {
  ...
  if (IsGraphics()) {
    if (output_glsl) {
      ...
      linked_program_->getEntryPointCode(stage_to_entry_point_index_[VERTEX_STAGE_IDX], 1, vert_code.writeRef(),
                                         diagnostics.writeRef());
      if (diagnostics) {
        LOGGER.Error(logcat::Resource, "Shader编译失败: {}",
      ...
      diagnostics = nullptr;
      linked_program_->getEntryPointCode(stage_to_entry_point_index_[FRAGMENT_STAGE_IDX], 1, vert_code.writeRef(),
                                         diagnostics.writeRef());
      if (diagnostics) {
        LOGGER.Error(logcat::Resource, "Shader编译失败: {}",
                     static_cast<const char *>(diagnostics->getBufferPointer()));
    ...
    Slang::ComPtr<slang::IBlob> diagnostics = nullptr;
    linked_program_->getEntryPointCode(stage_to_entry_point_index_[VERTEX_STAGE_IDX], 0, vert_code.writeRef(),
                                       diagnostics.writeRef());
    if (diagnostics) {
    ...
    diagnostics = nullptr;
    linked_program_->getEntryPointCode(stage_to_entry_point_index_[FRAGMENT_STAGE_IDX], 0, frag_code.writeRef(),
                                       diagnostics.writeRef());
    if (diagnostics) {
      LOGGER.Error(logcat::Resource, "Shader编译失败: {}", static_cast<const char *>(diagnostics->getBufferPointer()));
      return;
    ...
    shader_handles_[VERTEX_STAGE_IDX] = ctx->CreateShader(static_cast<const char *>(vert_code->getBufferPointer()),
                                                          vert_code->getBufferSize(), vert_name);
    shader_handles_[FRAGMENT_STAGE_IDX] = ctx->CreateShader(static_cast<const char *>(frag_code->getBufferPointer()),
                                                            frag_code->getBufferSize(), frag_name);
    is_compiled_ = true;
  }
}

bool Shader::IsCompute() const { return annotations_[GetEnumValue(ShaderAnnotation::Pipeline)] == 2; }
...
bool Shader::FillGraphicsPSODescFromShader(GraphicsPipelineDesc &pso_desc, bool output_glsl) {
  ...
  if (anno[GetEnumValue(ShaderAnnotation::Blend)] == 1) {
