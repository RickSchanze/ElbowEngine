...
Millisecond WorldClock::GetDeltaTime() const
{
    return delta_time_;
}

void WorldClock::SetGlobalTimeScale(float scale)
{
    global_time_scale_ = Math::Clamp(scale, 0.f, scale);
}


float WorldClock::GetGlobalTimeScale() const
{
...
void WorldClock::UnRegisterTick(ITick* tick)
{
    ranges::remove(ticks_, tick);
}

WorldClock& func::GetWorldClock()
{
    return WorldClock::GetByRef();
}

Millisecond func::GetDeltaTime()
{
    return GetWorldClock().GetDeltaTime();
}