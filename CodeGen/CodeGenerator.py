...
class CodeGenerator:
    ...
    def generate(self, original_header_name: str):
...
private: \\''')
        ...
        for struct in self.result.structs:
            generated_header_codes.append(f"""#undef GENERATED_BODY_{original_header_name}{struct.name}
#define GENERATED_BODY_{original_header_name}{struct.name} \\
RTTR_REGISTRATION_FRIEND \\
typedef {struct.name} ThisStruct; \\""")
        ...
        for class_ in self.result.classes + self.result.structs:
            ...
            generated_header_codes.append(f"""#undef GENERATED_SOURCE_{original_header_name}{class_.name}
#define GENERATED_SOURCE_{original_header_name}{class_.name} \\
rttr::registration::class_<{class_.full_name}>(\"{class_name}\").constructor<>()(rttr::policy::ctor::as_raw_ptr) \\""")
        ...
        for enum in self.result.enums:
            ...
            generated_header_codes.append(f"""#undef GENERATED_SOURCE_{original_header_name}{enum.name}
#define GENERATED_SOURCE_{original_header_name}{enum.name} \\
rttr::registration::enumeration<{enum.full_name}>(\"{enum_name}\")( \\""")
            for idx, value in enumerate(enum.constants):
                ...
                if idx != len(enum.constants) - 1:
                    generated_header_codes.append(f'rttr::value("{enum_value_name}", {enum.full_name}::{value.name}), \\')
                else:
                    generated_header_codes.append(f'rttr::value("{enum_value_name}", {enum.full_name}::{value.name}) \\')
            generated_header_codes.append("); \\")
            generated_header_codes.append(";")
...
#define GENERATED_SOURCE_{original_header_name} \\''')
        for class_ in self.result.classes + self.result.structs:
            generated_header_codes.append(f"GENERATED_SOURCE_{original_header_name}{class_.name} \\")
        for enum in self.result.enums:
            generated_header_codes.append(f"GENERATED_SOURCE_{original_header_name}{enum.name} \\")
        generated_header_codes.append(";")
