...
void ClassEntity::GenerateField(
    ...
    std::string RegisterStr = std::vformat(
        R"(.property("{}", &{}::{}))",
        std::make_format_args(FieldName, GetClassQualifiedName(), FieldDeclName)
    );
...
void EnumEntity::Generate(const clang::ASTContext* Context, llvm::raw_fd_ostream& OS) const {
    std::string CodeBody;
    CodeBody += "(";
    for (const auto* ConstantDecl: mConstants) {
        CodeBody += std::vformat(
            ...
            std::make_format_args(
                ConstantDecl->getNameAsString(), GetEnumName(), ConstantDecl->getNameAsString()
            )
        );
    }
    // 删除最后一个","
    ...
    const std::string EnumBody = std::vformat(
        R"(
#define GENERATED_ENUM_CODE_{}_{} \
    rttr::registration::enumeration<{}>("{}") \
    {};
)",
        std::make_format_args(mCurrentFileID, GetEnumName(), GetEnumName(), GetEnumName(), CodeBody)
    );
    OS << EnumBody;
}

std::string EnumEntity::GetEnumName() const {
...
void ClassEntity::Generate(const clang::ASTContext* context, llvm::raw_fd_ostream& os) const {
    const std::string CodeMacroStr = std::vformat(
        R"(#define GENERATED_CODE_{}{} \)",
        std::make_format_args(mCurrentFileID, GetClassName())
    );
    os << CodeMacroStr << "\n";
...
private:)",std::make_format_args(mCurrentFileID, GetClassName(), ThisClassName, ParentClassNameTypedef , RTTREnable,
                              mCurrentFileID, GetClassName()));
    // clang-format on
